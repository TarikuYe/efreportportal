...
export async function DELETE(request: Request) {
  try {
    ...
    if (error) {
      console.error('[correspondence] DELETE error:', error.message)
      return NextResponse.json({ error: 'Failed to delete correspondence: ' + error.message }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('[correspondence] DELETE unexpected:', err)
    return NextResponse.json({ error: 'Unexpected server error.' }, { status: 500 })
  }
}
